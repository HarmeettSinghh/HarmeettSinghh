...
if not ACCESS_TOKEN or not USER_NAME:
    ...
    raise SystemExit(
        ...
        '. Configure them before running today.py.'
    )
...
def loc_counter_one_repo(
    ...
    """
    Recursively call recursive_loc since GraphQL can only
    search 100 commits at a time.
    Only adds LOC value of commits authored by me.
    """
...
def cache_builder(
    ...
    since the last time it was cached.
    """
    ...
    for index in range(len(edges)):
        ...
        if repo_hash == hashlib.sha256(
            ...
            try:
                ...
                if int(commit_count) != edges[index]['node'][
                    'defaultBranchRef'
                ]['target']['history']['totalCount']:

                    owner, repo_name = edges[index][
                        'node'
                    ]['nameWithOwner'].split('/')
...
def force_close_file(data, cache_comment):
    """
    Forces the file to close, preserving whatever data
    was written to it.
    """
...
def stars_counter(data):
    ...
    for node in data:
        ...
        stargazers = repository.get('stargazers') if repository else None
...
def svg_overwrite(
    ...
    commits, stars, repositories, and lines written.

    Also enforces correct ASCII art styling for each theme:
      - dark_mode.svg: ASCII fill #c9d1d9 (light text on dark background)
      - light_mode.svg: ASCII fill #24292f + font-weight=bold (dark bold text on
        light background so tiny dot characters remain clearly visible).
        Removes any leftover dark panel rects that must not appear in light mode.
    """
    ...
    # ── Enforce theme-specific ASCII art styling ──────────────────────
    is_light = 'light_mode' in filename

    # Remove any dark panel rects from light mode (safety guard)
    if is_light:
        for child in list(root):
            tag = child.tag.split('}')[-1] if '}' in child.tag else child.tag
            if tag == 'rect' and child.get('fill') == '#161b22':
                root.remove(child)

    # Set correct ASCII art text attributes
    texts = root.findall('.//{%s}text' % SVG_NS)
    if texts:
        ascii_text = texts[0]
        if is_light:
            ascii_text.set('fill', '#24292f')
            ascii_text.set('font-weight', 'normal')
        else:
            ascii_text.set('fill', '#c9d1d9')
            # Remove font-weight if it was ever set on dark mode
            if 'font-weight' in ascii_text.attrib:
                del ascii_text.attrib['font-weight']

    # ── Update dynamic data fields by ID ─────────────────────────────
...
def commit_counter(comment_size):
    ...
    using the cache file created by cache_builder.
    """
